...
genepanels = snakemake.input[2]
mosdepth = snakemake.input[4]  # Summary file
...
while i <= (len(tableLinesCov)-1):
	...
	if (i == (len(tableLinesCov) - 1)) or (tableLinesCov[i][0] != tableLinesCov[i+1][0]):
		...
		covRow = [gene, transcript, round((cov/length), 2), round((minBredth*100), 1), round((medBreadth*100), 1), round((maxBreadth*100), 1)]
...
for line in panels:
    ...
    worksheetpanel.write('A1', 'Average coverage and coverage breadth per gene for '+line, headingFormat)
    ...
    with open(panel, 'rt') as file:
        for lline in file:
            while i < len(covtable):
                ...
                if lline.rstrip() == covtable[i][0]:
                    ...
                    i=len(covtable)
        worksheetpanel.write('A4', 'Averge coverage and coverage breadth of the '+str(number)+' genes in '+line+'gene panel.')
        ...
        worksheetOver.write_url((16+number), 0, "internal: '"+line+"'!A1", string='Coverage analysis of '+line+' gene panel')
...
worksheetLowCov.write('A4', 'Gene regions with coverage lower than '+str(minCov)+'x.')
...
worksheetCov.add_table(tableArea, {'data': lowRegLines, 'columns': headerListDict, 'style': 'Table Style Light 1'})
